...
var Drink = function (pantry, drinkOrder) {
    ...
    for (var userPreference in drinkOrder) {
        //generate a random number in order to pick up on ingredient in each ingredients category (for example choose "slug of whiskey")
        ingredientNumber = generateRandomNumber(0, 2);
        if (drinkOrder[userPreference]) {
            //for ea one of the ingredient categories, choose one random ingredient and populate the ingredientArray with it
...
$(document).ready(function () {
    ...
    $('form').on('submit', function (event) {
        ...
        //check if ea one of the ingredient types have been chosen and to the orderValues array;
        ...
        //use the 2 constructors to creat 2 new objects
        drinkOrder = new Order(orderValues); //create new order from existing user choice
        concoction = new Drink(pantry, drinkOrder); //mis drink with Drink constructor

        //if there is at least one ingredient selected then show the concoction
        if (concoction.length > 0) {

            //build the chose ingredients from the ingredients array
            ...
            $(".ouput h3").html("Sail ho! Here's yer drink!" + drinkNamer(concoction) + "Grog, ye scurvy dog!");
        ...
        //if there are no ingredients selected then show alert
            ...
            alert('Shiver me timbers! What yer like to drink?');
        }
    });
});
